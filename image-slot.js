...
(() => {
  ...
  // Persistent tombstones: ids the user deleted AFTER hydration. The sidecar
  // may be too big to rewrite (2MB cap), so a delete that only lands in
  // IndexedDB would be resurrected on the next load by the stale sidecar's
  // copy. Recording deletions here (localStorage) makes them stick: the
  // merge strips these ids and migration skips them, whatever the sidecar says.
  const PT_KEY = 'image-slots-deleted';
  let ptomb;
  try { ptomb = new Set(JSON.parse(localStorage.getItem(PT_KEY) || '[]')); }
  catch (e) { ptomb = new Set(); }
  function savePtomb() {
    try { localStorage.setItem(PT_KEY, JSON.stringify([...ptomb])); } catch (e) {}
  }
  let loaded = false;
  let loadP = null;

  // ── Persistence (IndexedDB primary, sidecar best-effort) ────────────────
  // The host permits writing exactly ONE sidecar file
  // (.image-slots.state.json) and caps it near 2MB — a portfolio's worth of
  // images cannot live there, and every other filename is rejected. So
  // IndexedDB (hundreds of MB, per-key) is the source of truth across
  // reloads; the sidecar is still mirrored best-effort as that one file so
  // small sets keep showing on share links / exports, and its failure is
  // silent (an upload is never lost — IndexedDB already holds it).
  const DB_NAME = 'image-slots-store';
  const STORE = 'slots';
  let dbP = null;
  function idb() {
    if (dbP) return dbP;
    dbP = new Promise((res) => {
      let req;
      try { req = indexedDB.open(DB_NAME, 1); } catch (e) { res(null); return; }
  ...
  function idbPut(id, val) {
    return idb().then((db) => db && new Promise((res) => {
      try {
        ...
        if (val == null) tx.objectStore(STORE).delete(id);
        else tx.objectStore(STORE).put(val, id);
  ...
  function idbAll() {
    return idb().then((db) => db && new Promise((res) => {
      ...
      try {
        ...
        const kReq = st.getAllKeys(); const vReq = st.getAll();
        tx.oncomplete = () => {
          const ks = kReq.result || []; const vs = vReq.result || [];
          ks.forEach((k, i) => { out[k] = vs[i]; });
          res(out);
        };
        tx.onerror = () => res({}); tx.onabort = () => res({});
      } catch (e) { res({}); }
    }) || {});
  }
  ...
  function load() {
    ...
    loadP = (async () => {
      // 1. IndexedDB — authoritative across reloads.
      const idbSlots = (await idbAll()) || {};
      // 2. Sidecar — author-provided images + one-time migration source.
      //    Handles the legacy monolith, the interim shard manifest, and the
      //    plain single-file shape saveSidecar() writes below.
      let disk = {};
      try {
        const j = await fetch(STATE_FILE).then((r) => (r.ok ? r.json() : null));
        if (j && typeof j === 'object') {
          if (Array.isArray(j.__shards)) {
            const parts = await Promise.all(j.__shards.map((s) =>
              fetch('.image-slots.' + s + '.state.json').then((r) => (r.ok ? r.json() : null)).catch(() => null)));
            parts.forEach((d) => { if (d && typeof d === 'object') Object.assign(disk, d); });
          } else if (!('__shards' in j)) {
            disk = j;
          }
        }
      } catch (e) {}
      // Merge: sidecar < IndexedDB < in-memory changes that raced ahead
      // of the read (a drop or clear), so neither is clobbered by hydration.
      const merged = Object.assign({}, disk, idbSlots, slots);
      for (const k in slots) {
        if (merged[k] && !merged[k].u && (idbSlots[k] || disk[k])) {
          const src = idbSlots[k] || disk[k];
          merged[k].u = typeof src === 'string' ? src : src.u;
        }
      }
      for (const id of tombstones) delete merged[id];
      for (const id of ptomb) delete merged[id];
      slots = merged;
      tombstones.clear();
      // One-time migration: copy any sidecar-only image into IndexedDB —
      // but never a user-deleted id (that would revive it).
      for (const k in disk) { if (!(k in idbSlots) && !ptomb.has(k)) idbPut(k, slots[k]); }
    })().catch(() => {}).then(() => {
      loaded = true;
      subs.forEach((fn) => fn());
      saveSidecar();
  ...
  // Unload-time flush posts the current slots to the sidecar immediately
  // (best-effort): the debounced saveSidecar timer would never fire in an
  // unloading document. IndexedDB writes from setSlot have already
  // committed, so this only refreshes the export mirror.
  // Best-effort single-file sidecar mirror, debounced. Silent on failure —
  // IndexedDB is the real store, so a >2MB rejection only means large sets
  // won't appear on share/export until deploy, never that an upload is lost.
  ...
  function setSlot(id, val) {
    if (!id) return;
    if (val) { slots[id] = val; tombstones.delete(id); if (ptomb.delete(id)) savePtomb(); }
    else { delete slots[id]; if (!loaded) tombstones.add(id); if (!ptomb.has(id)) { ptomb.add(id); savePtomb(); } }
    subs.forEach((fn) => fn());
    // Write straight to IndexedDB (authoritative, no size cap). Then mirror
    // to the sidecar best-effort. Gate on the initial read so a change made
    // before hydration isn't lost by the merge; load() preserves it.
    if (loaded) { idbPut(id, val || null); saveSidecar(); }
    else load().then(() => { idbPut(id, slots[id] || null); saveSidecar(); });
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '  <button data-act="clear" title="Delete image">\u{1F5D1}\uFE0E</button></div>' +
    ...
    handleEvent(e) {
      // Only intercept FILE drags (image uploads). In-page element drags
      // (tile reordering) must pass through to the host page's handlers.
      const isFileDrag = e.dataTransfer &&
        Array.prototype.indexOf.call(e.dataTransfer.types || [], 'Files') >= 0;
      if (!isFileDrag && e.type !== 'dragleave') return;
